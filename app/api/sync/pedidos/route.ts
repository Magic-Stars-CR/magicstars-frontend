...
export async function POST(request: NextRequest) {
  try {
    console.log('🔄 Proxy: Sincronizando pedidos...');
    
    const response = await fetch(`${RAILWAY_URL}/webhook/sincronizar-pedidos`, {
      method: 'POST',
      headers: {
    ...
    console.log('✅ Sincronización de pedidos exitosa');
    return NextResponse.json(data);
  } catch (error) {
    console.error('❌ Error en proxy de sincronización de pedidos:', error);
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({ message: 'Ruta de sincronización de pedidos activa' });
}
